...
function PcImplantExperienceCalculator() {
  ...
  useEffect(() => {
    const options = Object.keys(experienceData).map((level) => ({
      label: `等级 ${level}`,
      value: level,
    }));
  ...
  const handleLevelChange = (changedValues) => {
    if (changedValues.currentLevel) {
      const level = parseInt(changedValues.currentLevel);
      setMaxCurrentExp(experienceData[level].required - 1);
      form.setFieldsValue({ currentExp: 0 });
    }
  };
  ...
  const calculateExperience = () => {
    ...
    const currentLevelNumber = parseInt(currentLevel);
    const targetLevelNumber = parseInt(targetLevel);
  ...
  return (
    ...
    >
          ...
          <Form.Item label="目标等级" name="targetLevel">
            <Select
              ...
              options={levelOptions.filter(
                (opt) =>
                  parseInt(opt.value) >
                  parseInt(form.getFieldValue("currentLevel") || 0)
              )}
              showSearch
              optionFilterProp="label"
              disabled={!form.getFieldValue("currentLevel")}
            />
          </Form.Item>
        </Col>
      </Row>
      ...
      <Form.Item>
        <Space>
          <Button type="primary" onClick={calculateExperience}>
            计算
          </Button>
          <Button
            onClick={() => {
      ...
      {bestPlan.length > 0 && (
        <ResultSection>
          <div style={{ marginBottom: 16, fontWeight: 500 }}>{result}</div>
          <StyledTable
            columns={columns}
            dataSource={bestPlan}
            pagination={false}
...
export default PcImplantExperienceCalculator;
