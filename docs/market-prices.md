...
- `backend/Market/session_bundle.py` 加载 0600 且属于采集用户的私有 JSON 会话模板，单账号路径由 `MARKET_SESSION_FILE` 指定，多账号池由 `MARKET_SESSION_FILES`（按 Linux `:` 分隔）指定。每轮只随机选一个已验证模板，不记录账号标识。模板含可重放的登录材料，不是公开配置；不得放进 Git、发布包、网页、普通日志或数据库。当前实验性协议能在每轮用**已取得的会话模板**重新连接和登录角色，但**不能仅凭邮箱/密码完成首次认证或自动续期**。会话过期会停用自动重试并显示 `needs_auth`，管理员更换模板后可手动排队验证。
