...
Review checkpoint: report files, tests and unresolved baseline failures. GitHub CLI login is currently unavailable. Do not change the live server before this checkpoint and the gates below.
...
- Existing production requirements checksum matches the repository; `pip check` reports no broken requirements. Next: GitHub-hosted CI, baseline/config backup, dedicated deploy identity, then controlled switch/rehearsal with auto-deploy still disabled.
