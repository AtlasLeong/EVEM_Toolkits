...
All free-text writes (claim name/short name/statement/contact, revision text/benefits note, review and visibility reasons) reject lone Unicode surrogates with a field-specific 400 before persistence. Valid non-BMP characters such as emoji remain supported. Rejected writes leave row content, version, ownership and moderation state unchanged. Read serializers tolerate malformed historical strings by returning empty text; malformed non-string revision text fields also read as empty text. This is a response-only projection, including private claim metadata and nested location text: reads never repair, rewrite or delete stored history, and one malformed record must not make a whole management/review/public list fail UTF-8 rendering.
...
All text fields reject unpaired Unicode surrogates before persistence, so JSON escape sequences cannot commit content that later fails UTF-8 rendering. Historical custom tags with these invalid characters are omitted on reads; malformed activity-description strings read as empty text without changing the raw snapshot or its overview kind.
...
- `GET claims/{id}/`: applicant/staff only; claim includes nested current corporation identity, `proposed_name` / `proposed_short_name` (nullable strings), and decision fields. These proposed fields also appear in private claim creation/retry, mine and staff review responses, never in public corporation objects.
...
### Independent claim identity proposals

Every new named application stores its own NFKC-normalized name and trimmed short name as `proposed_name` and `proposed_short_name`. Two applicants for the same normalized name share the stable corporation ID/name key but retain independent proposals; creating, rejecting or retrying an application never overwrites another proposal or the corporation's current identity. A new application using `corporation_id` snapshots that corporation's known name/short name at submission; the existing request shape does not allow a rename or a client-supplied proposal field.

Only approving a pending claim for a still-unowned corporation adopts that claim's name/short name and owner in one transaction under the existing user -> corporation -> claim locks. The proposal must resolve to the same canonical name key; a mismatched imported/corrupt proposal is a 409, not an identity change. Once another claim has won, any later approval is 409 without changing ownership or identity. A rejected wrong abbreviation therefore does not poison a later correctly named application. Proposal values are shown explicitly to reviewers instead of presenting the first applicant's abbreviation as the candidate's own.

Migration `0003_claim_identity_proposal` adds only two nullable columns. Existing applications stay null, without guessing/backfilling their original proposals from the corporation. Approving such a legacy claim preserves the current known identity. An empty proposed short name is intentional and distinct from a null legacy snapshot. Apply the additive schema migration before starting code that reads these columns; older code can run against the expanded schema, so code rollback should retain the extra columns/data rather than reverse the migration.
...
- `POST reviews/claims/{id}/decision/`: staff `{decision:approve|reject,reason}`. Lock users, corporation and claim in order; pending only; approve verifies owner still null then atomically assigns applicant and applies that claim's independent identity proposal (legacy null keeps current identity). Reject requires reason. Both persist reviewer/time/reason, never auto-publish.
