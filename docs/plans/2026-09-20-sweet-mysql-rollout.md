...
- [x] 导入工具测试、规格审查、代码审查
- [x] 全量 MySQL 演练：逐表行数、内容摘要一致
- [x] 正式静态版本库导入与独立再次校验
- [x] 导入后业务健康复核

## 完成证据（UTC 2026-09-20）

- Rehearsal schema `evem_sweet_218811_c47d33925de2d61c_r1` 独立回读为
  `verified`，22 表 / 899,630 行，源 SHA 与本地清单一致；该演练库保留，未自动删除。
- 正式隔离 schema `evem_sweet_218811_c47d33925de2d61c` 导入后再次执行只读
  `verify`，返回 `verified`、22 表 / 899,630 行、源 SHA
  `c47d33925de2d61c44880dfc2fd45e41ada168c596961deb696ecefc8818dd6a`。
- 导入前后 `evem-backend.service` 均为 `active`；没有修改 `eve_echoes`、`license`、
  Django aliases、发布版本或服务进程。原始包、归档清单和业务备份仍在 root-only 私有目录。
- 业务库备份 `business-before-static-import.sql`（27,219,274 bytes）仍标记为
  `restored: false`；本次静态导入不需要业务回滚。
