...
> **自动部署已启用（2026-09-08）。日常只需将待发布代码推送到 master，GitHub Actions 会测试、打包、部署并检查健康状态。** 操作与回滚入口见 [自动部署运行说明](scripts/deploy/README.md)，实测证据见 [上线记录](docs/plans/2026-09-08-deployment-rollout-record.md)。不要再向旧项目目录覆盖 dist 或 git pull 更新线上版本。**下文仅为旧版手工部署历史参考，不适用于当前线上发布流程。**
