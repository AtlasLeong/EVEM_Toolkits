...
const backgrounds = ['远征舰队', '星环巨行星', '旋臂星河', '轨道船坞', '黑洞视界', '创生星云', '冰封边境', '战舰残骸'];
...
test('海报在独立弹窗打开，页面宽度不变且关闭后恢复焦点', async ({ page }) => {
  ...
  await expect(dialog.getByRole('button', { name: '导出 PNG', exact: true })).toBeEnabled();
...
test('八款背景及三种模板均可导出有效且不同的PNG', async ({ page }) => {
  ...
  for (const label of backgrounds) {
    ...
    for (const template of ['招募海报', '军团介绍', '活动宣传']) {
...
test('背景加载失败时禁止导出，重试后恢复', async ({ page }) => {
  await communityFixture(page);
  let fail = true;
  await page.route('**/posters/*', route => fail && !route.request().url().includes('-thumb') ? route.abort() : route.continue());
  await page.goto('/corporations/1');
  await page.getByRole('button', { name: '制作海报', exact: true }).click();
  const dialog = page.getByRole('dialog', { name: '制作军团海报' });
  await expect(dialog.getByRole('alert')).toBeVisible();
  await expect(dialog.getByRole('button', { name: '导出 PNG', exact: true })).toBeDisabled();
  fail = false;
...
test('手机海报全屏且没有水平溢出，关闭后可继续浏览', async ({ page }) => {
  ...
  await expect(dialog).not.toBeVisible();
  await expect(page.getByRole('heading', { name: '关于军团', exact: true })).toBeVisible();
});
...
test('快速切换背景不会导出旧画面，焦点始终留在弹窗内', async ({ page }) => {
  ...
  await page.route('**/posters/black-hole.webp', async route => { await delayed; await route.continue(); });
  await page.goto('/corporations/1');
  await page.getByRole('button', { name: '制作海报', exact: true }).click();
  const dialog = page.getByRole('dialog', { name: '制作军团海报' });
  const button = dialog.getByRole('button', { name: '导出 PNG', exact: true });
  ...
  await expect(button).toBeEnabled();
  release();
  await expect(dialog.locator('canvas')).toHaveAttribute('data-background', 'wreckfield');
