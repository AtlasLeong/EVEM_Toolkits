...
test("eight original backgrounds replace every legacy option", () => {
  ...
  assert.equal(
    normalizePosterBackground("not-a-background"),
    "expedition-fleet",
  );
...
test("overview poster uses ongoing activities and never revives a historical event", () => {
  for (const description of ["主权战与反收割，周末小队游猎。", ""]) {
    const rendered = [];
    const ctx = new Proxy(
      {
    ...
    assert.match(all, /主要活动/);
    assert.match(all, /远航军团/);
    assert.match(all, /反收割/);
    assert.match(all, /德里克/);
    assert.match(all, /每晚在线/);
    assert.match(all, /未审核/);
    assert.doesNotMatch(all, /过期远征|旧日期|旧集结点|旧说明|集结时间待定/);
    if (description) assert.ok(all.includes(description));
  }
});
...
test("approved posters omit platform branding while retaining corporation content", () => {
  for (const template of Object.keys(POSTER_TEMPLATES)) {
    ...
    drawCorporationPoster(
      ...
      {
        public_contact: "游戏内联系招募官",
        activity_description: "日常反收割",
        event_title: "周末远征",
        introduction: "共同探索星海",
      },
      template,
    ...
    assert.ok(
      !rendered.some((value) =>
        /EVEM|军团资料已审核|活动信息请向军团确认/.test(value),
      ),
      "platform branding must not be printed on the poster",
    );
...
test("legacy event posters keep historical information explicitly identified", () => {
  const rendered = [];
  const ctx = new Proxy(
    {
      measureText: (value) => ({ width: measure(value) }),
  ...
  const all = rendered.join("\n");
  for (const item of ["旧版活动资料", "旧远征", "旧日期", "旧地点", "旧说明"])
    assert.ok(all.includes(item));
});
...
test("every background draws the supplied artwork without repeated grid or orbital ornaments", () => {
  ...
  assert.throws(
    () => drawPosterBackground(createContext(), "expedition-fleet"),
    /背景/,
  );
});
    ...
    for (const template of Object.keys(POSTER_TEMPLATES)) {
      ...
      drawCorporationPoster(
        ...
        { name: "远航军团" },
        {
          ...
          base_location: {
            region_name: "德里克",
            constellation_name: "阿玛",
            solarsystem_name: "测试星系",
          },
          public_contact: "请联系招募官",
          event_time: "今晚20:00",
          event_location: "集结星系",
        },
        template,
      ...
      if (template !== "event")
        assert.ok(rendered.some(({ value }) => value.includes("测试星系")));
    }
  }
});
