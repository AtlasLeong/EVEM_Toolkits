...
test('pirate map system hit opens a prefilled report while target cards still open details', async ({ page }) => {
  ...
  await expect(workspace.locator('[data-pirate-system="102"]')).toBeVisible()
  await workspace.locator('[data-pirate-system="102"]').click()
  const form = page.getByRole('dialog', { name: '上报目标线索' })
  await expect(form).toContainText('已选：德里克二')
  ...
  await page.mouse.move(beforeDrag.x + beforeDrag.width / 2 + 100, beforeDrag.y + beforeDrag.height / 2, { steps: 4 })
  await page.mouse.up()
  await expect.poll(async () => (await systemHit.boundingBox()).x).toBeGreaterThan(beforeDrag.x + 50)
